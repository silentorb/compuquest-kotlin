...
import silentorb.mythic.platforming.AudioSourceLocationGetter
...
class DesktopAudio : PlatformAudio {
	...
	fun setSourcePosition(source: Int, location: Vector3) {
		alSource3f(source, AL_POSITION, location.x.toFloat(), location.y.toFloat(), location.z.toFloat())
	}

	override fun play(buffer: Int, volume: Float, location: Vector3?): Int? {
		...
		return if (sourceIndex == -1)
			...
			if (location != null) {
				setSourcePosition(source, location)
	...
	override fun update(gain: Float, listenerLocation: Vector3?) {
		if (gain != this.gain) {
			this.gain = gain
		...
		if (listenerLocation != null) {
			alListener3f(
				AL_POSITION,
				listenerLocation.x.toFloat(),
				listenerLocation.y.toFloat(),
				listenerLocation.z.toFloat()
			)
		}
	...
	override fun update(gain: Float, getSourceLocation: AudioSourceLocationGetter) {
		if (gain != this.gain) {
			this.gain = gain
			alListenerf(AL_GAIN, gain)
		}
		...
		sources.forEachIndexed { index, source ->
			if (alGetSourcei(source, AL_SOURCE_STATE) == AL_STOPPED) {
				...
				val location = getSourceLocation(source)
				setSourcePosition(source, location)
			}
		}
		val error = alGetError()
