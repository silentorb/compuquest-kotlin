...
import compuquest.population.traversableSides
...
fun checkBlockMatch(surroundingSides: SideMap, getBlock: GetBlock,
  ...
  val match = blockCells
    .firstOrNull { baseOffset ->
      ...
      essentialCellSide != null && essentialCellSide.isTraversable == traversable && essentialCellSide.canMatchEssential &&
          block.cells
            .all { (cellOffset, cell) ->
...
fun matchConnectingBlock(dice: Dice, blocks: Set<Block>, grid: BlockGrid, location: Vector3i,
                         essentialDirectionSide: Direction
): Pair<Vector3i, Block>? {
